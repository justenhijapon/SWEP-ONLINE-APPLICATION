...
@endphp
<form id="edit_port_form_{{$rand}}" autocomplete="off">
    @csrf
...
{{--                ],$port ?? null) !!}--}}
    ...
    <div class="modal-footer">
        ...
        <button type="submit" class="btn {!! __static::bg_color(Auth::user()->color) !!} pull-right update_port_btn"> <i class="fa fa-fw fa-save"></i> Save</button>
    </div>
</form>
...
<script type="text/javascript">
    ...
    $("#edit_port_form_{{$rand}} .autonum").each(function(){
        new AutoNumeric(this, autonum_settings);
    })

    $('#edit_port_form_{{$rand}} .select2').select2();

    $("#edit_port_form_{{$rand}}").submit(function(e){
        let form = $(this);
        e.preventDefault();
        let uri = "{{ route('dashboard.port.update', 'slug') }}";
